...
const PlayQuizContent = () => {
    ...
    useEffect(() => {
        ...
        if (timeLeft === 0) { handleNextQuestion(true); return; }
    ...
    const handleNextQuestion = (force = false) => {
        ...
        if (!hasTimePerQuestion && !force && !userAnswers[currentQuestionIdx]) {
            toast.error("Please select an answer before proceeding"); return;
        }
    ...
    const handleJoinQuiz = async () => {
        ...
        if (localStorage.getItem(lockKey) === "SUBMITTED") {
            toast.error("ACCESS DENIED: You have already submitted this exam."); return;
        }
        setIsLoading(true);
        try {
            ...
            if (!data.questions || data.questions.length === 0) {
                toast.error("ERROR: THIS QUIZ HAS NO QUESTIONS"); setIsLoading(false); return;
            }
                            ...
                            <FormGrid>
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        ...
                                        <input type="text" placeholder="Enter full name..." value={joinData.participantName}
                                ...
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        <User size={16} className="input-icon" />
                                        <input type="email" placeholder="example@email.com" value={joinData.email} required
                                ...
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        ...
                                        <input type="number" placeholder="000000" value={joinData.quizId}
                                ...
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        ...
                                        <SelectField value={joinData.studentClass} $hasValue={!!joinData.studentClass}
                                ...
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        ...
                                        <SelectField value={joinData.division} $hasValue={!!joinData.division}
                                ...
                                <InputGroup>
                                    ...
                                    <div className="input-wrapper">
                                        ...
                                        <input type="number" placeholder="Enter roll number..." value={joinData.rollNumber}
                                        ...
                                        <QuestionCard key={idx} $isSubmitted={isSubmitted}>
                                            ...
                                            <OptionsGrid>
                                                {["opt1", "opt2", "opt3", "opt4"].map((optKey) => {
                                                    ...
                                                    return (
                                                        <OptionButton key={optKey} variant={variant} onClick={() => handleSelectOption(idx, optValue)}>
                                ...
                                <NavButtonGroup>
                                    ...
                                    <PrimaryButton onClick={handleNextQuestion} disabled={isLoading} style={{ flex: 1 }}>
                                        {isLoading ? (
                                            ...
                                            <>{currentQuestionIdx === quizData.questions.length - 1 ? "FINISH" : "NEXT"}<ChevronRight size={20} /></>
                                        )}
                                    </PrimaryButton>
                                </NavButtonGroup>
                            ) : (
...
const RuleItem = styled.div`
    ...
    background: ${({ theme }) => theme.bgRule};
    border: 1px solid ${({ theme }) => theme.border};
    transition: all 0.3s ease;
    &:hover { border-color: ${({ theme }) => theme.borderHover}; transform: translateY(-2px); }
    .rule-header {
        display: flex; align-items: center; gap: 10px;
        color: ${({ theme }) => theme.text}; font-weight: 700; font-size: 13px;
...
const EntryWrapper = styled.div`
    ...
    animation: ${fadeIn} 0.6s ease-out; margin-top: -10px;
`;
...
const InputGroup = styled.div`
    ...
    .input-wrapper {
        ...
        input {
            width: 100%; background: ${({ theme }) => theme.bgInput};
            border: 1px solid ${({ theme }) => theme.border};
            padding: 16px 16px 16px 48px; color: ${({ theme }) => theme.text};
            font-size: 14px; transition: all 0.3s ease;
            &:focus { outline: none; border-color: ${({ theme }) => theme.text}; }
...
const SelectField = styled.select`
    ...
    color: ${props => props.$hasValue ? props.theme.text : props.theme.textMuted};
...
const EntryButton = styled.button`
    ...
    border: none; padding: 20px; font-weight: 900; font-size: 13px;
...
const QuizHeader = styled.div`
    ...
    .top-meta {
        ...
        .status-pill {
            ...
            padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 700; transition: all 0.3s ease;
            &.timer { border: 2px solid ${({ theme }) => theme.text}; color: ${({ theme }) => theme.text}; background: transparent; }
            &.score { background: ${({ theme }) => theme.btnBg}; color: ${({ theme }) => theme.btnText}; }
        }
    }
    h2 { font-size: 1.6rem; font-weight: 900; margin: 0; line-height: 1.3; text-transform: uppercase; letter-spacing: 0.5px; color: ${({ theme }) => theme.text}; }
`;

const ProgressBarContainer = styled.div`
    width: 100%; height: 6px; background: ${({ theme }) => theme.border};
    margin-bottom: 40px; border-radius: 3px; overflow: hidden;
`;

const ProgressFill = styled.div`
    height: 100%; width: ${props => props.progress}%;
    background: ${({ theme }) => theme.btnBg}; transition: width 1s linear;
`;

const ContentArea = styled.div`margin-bottom: 40px; animation: ${fadeIn} 0.4s ease-out;`;

const QuestionCard = styled.div`
    .q-label { font-size: 10px; font-weight: 800; color: ${({ theme }) => theme.textDim}; margin-bottom: 16px; text-transform: uppercase; letter-spacing: 1px; }
    h3 { font-size: 1rem; font-weight: 500; line-height: 1.6; margin-bottom: 40px; color: ${({ theme }) => theme.text}; }
    padding: 32px; background: ${({ theme }) => theme.bgCard};
    border: 1px solid ${({ theme }) => theme.border};
    border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.1); transition: all 0.3s ease;
    ${props => props.$isSubmitted && css` margin-bottom: 60px; padding-bottom: 40px; opacity: 0.9; `}
`;
...
const OptionButton = styled.div`
    ...
    background: ${({ theme }) => theme.bgOption};
    border: 2px solid ${({ theme }) => theme.borderOption};
    ...
    .opt-indicator {
        ...
        border: 2px solid ${({ theme }) => theme.indicatorBorder};
        border-radius: 50%; transition: all 0.3s ease; flex-shrink: 0;
        background: ${({ theme }) => theme.indicatorBg};
    ...
    .opt-text { font-size: 15px; font-weight: 500; color: ${({ theme }) => theme.textOption}; flex: 1; letter-spacing: 0.3px; }

    &:hover {
        border-color: ${({ theme }) => theme.borderHover};
        background: ${({ theme }) => theme.bgOptionHover};
        transform: translateY(-3px);
        .opt-indicator { border-color: ${({ theme }) => theme.text}; }
        .opt-text { color: ${({ theme }) => theme.textOptionHover}; }
    }

    ${props => props.variant === "selected" && css`
        border-color: ${({ theme }) => theme.borderSelected};
        background: ${({ theme }) => theme.bgOptionSelected};
        .opt-indicator {
            border-color: ${({ theme }) => theme.borderSelected};
            &::after { background: ${({ theme }) => theme.borderSelected}; }
        }
        .opt-text { color: ${({ theme }) => theme.text}; font-weight: 600; }
    `}
    ${props => props.variant === "correct" && css`
        border-color: #4ade80;
        background: linear-gradient(135deg, #0f3f0f 0%, #1a5a1a 100%);
        .opt-indicator { border-color: #4ade80; &::after { background: #4ade80; } }
        .opt-text { color: #fff; font-weight: 700; }
        .res-icon { color: #4ade80; }
    `}
    ${props => props.variant === "wrong" && css`
        border-color: #ff5555; opacity: 0.7;
...
const FooterActions = styled.div`
    display: flex; justify-content: center; padding-top: 30px; animation: ${fadeIn} 0.5s ease-out;
`;
...
const BackButton = styled.button`
    ...
    &:hover:not(:disabled) {
        border-color: ${({ theme }) => theme.text}; color: ${({ theme }) => theme.text}; transform: translateY(-2px);
    }
...
const SecondaryButton = styled.button`
    ...
    color: ${({ theme }) => theme.textDim};
    padding: 12px 24px; font-size: 12px; font-weight: 800;
...
const ModalIconBox = styled.div`
    width: 64px; height: 64px; border: 1px solid ${({ theme }) => theme.border};
    border-radius: 12px; display: flex; align-items: center; justify-content: center;
    margin: 0 auto 20px; background: ${({ theme }) => theme.bgCard};
`;
const ModalTitle = styled.h3`
    font-size: 1.2rem; font-weight: 900; letter-spacing: 2px;
    color: ${({ theme }) => theme.text}; margin: 0 0 24px; text-transform: uppercase;
`;
const ModalBody = styled.div`
    ...
    .stat-row {
        ...
        background: ${({ theme }) => theme.bgModalStat}; border: 1px solid ${({ theme }) => theme.border};
        .label { font-size: 11px; font-weight: 700; color: ${({ theme }) => theme.textMuted}; letter-spacing: 1px; text-transform: uppercase; }
        .value { font-size: 14px; font-weight: 800; color: ${({ theme }) => theme.text}; }
        .value.warn { color: #ff5555; }
    }
    .warn-text {
        font-size: 12px; color: ${({ theme }) => theme.textDim}; line-height: 1.6;
        margin-top: 16px; padding: 12px 16px;
        border: 1px solid ${({ theme }) => theme.border};
        border-radius: 6px; background: ${({ theme }) => theme.bgModalStat}; text-align: left;
    }
`;
const ModalActions = styled.div`display: grid; grid-template-columns: 1fr 1fr; gap: 12px;`;
const ModalCancelBtn = styled.button`
    background: transparent; border: 2px solid ${({ theme }) => theme.border};
    color: ${({ theme }) => theme.textDim};
